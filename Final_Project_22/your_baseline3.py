...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def __init__(self, index, timeForComputing = .1): # situation제외하고 captureAgents.CaptureAgent 의 def __init__그대로 가져옴
        self.index = index
        self.red = None
        ...
        self.situation = 'OffensiveSituation' # 초기값은 OffensiveSituation으로 상태 저장
    ...
    def getFeatures(self, gameState, action): # 상황에 따라 다른 feature 사용
        ...
        opponent = [gameState.getAgentState(oppo) for oppo in self.getOpponents(gameState)] # 적들 list
        invader = [oppo for oppo in opponent if oppo.isPacman] # 적이 pacman상태라면 invader임, 침입자 리스트
        
        if self.situation == 'OffensiveSituation': # OffensiveSituation상태일 때
            if invader: # 침입자가 있다면
                self.situation = 'Defense' # defense상태로 바꿈
                return self.getFeatures_Defense(gameState, action) # defens관련 feature 반환
            return self.getFeatures_usual(gameState, action) # 침입자가 없으면 offense관련 feature 반환
        
        if self.situation == 'Defense': # defense 상태일 때
            if invader: # defense상태일때 침입자가 있는 경우
                return self.getFeatures_Defense(gameState, action) # defens관련 feature 반환
            self.situation = 'OffensiveSituation' # 침입자가 없으면 offense 상태로 바꾸고 
            return self.getFeatures_usual(gameState, action) # offense관련 feature 반환
    
    def getWeights(self, gameState, action): # 상황에 따라 다른 weight 사용
        
        if self.situation == 'OffensiveSituation': # OffensiveSituation상태일 때
            return self.getWeights_usual(gameState, action) # offense관련 weight 반환
        elif self.situation == 'Defense': # defense 상태일 때
            return self.getWeights_defense(gameState, action) # defens관련 weight 반환
    ...
    def getFeatures_Defense(self, gameState, action):
    ...
    def getWeights_defense(self, gameState, action): # weight 등록
